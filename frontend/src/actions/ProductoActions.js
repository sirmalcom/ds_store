...
export const getProduct= (keyword="",currentPage=1,category) => async (dispatch)=>{
  try {
     ...
     let link = `/api/v2/productos?keyword=${keyword}&page=${currentPage}`;
      
     if(category){
      link = `/api/v2/productos?keyword=${keyword}&page=${currentPage}&categoria=${category}`;
     }
...
export const getProductDetails= (id) => async (dispatch)=>{
  try {
      ...
      const { data } = await axios.get(`/api/v2/producto/${id}`);
...
export const newReview = (reviewData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`/api/v2/product/resena`, reviewData, config);
